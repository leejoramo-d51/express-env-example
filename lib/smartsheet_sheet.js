...
module.exports.post_row = async function(req, res, schema) {
    ...
    for (key in data) {
        ...
        } else if (data.hasOwnProperty('hyperlinkUrl__'+key)) {
            cell['value'] = data[key]
            if (data['hyperlinkUrl__'+key] != '') {
            cell['hyperlink'] = {'url': data['hyperlinkUrl__'+key] }
            }
