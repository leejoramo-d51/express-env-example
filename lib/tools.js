...
const makeFormJSON = function(config) {
 ...
 module.exports.buildFormConfig = function (fieldArray) {
     ...
     for (let i = 0; i < fieldCount; i++) {
         ...
         fieldsJSON[item['title']] = thisField
     }

     const s = JSON.stringify(fieldsJSON, null, 4)
     return s
 }
 ...
 module.exports.buildFormSvelte = function(nameToId) {
     banner('FUNCTION:tools.buildFormSvelte')
     log('nameToId', nameToId)
     // saves a generic Jade/Pug template which can be edited
     // to customize
     let s = ''
     let template =  `
        <Row>
 ...
 module.exports.sheetNameHasUpdateMarker = function(sheetName) {
     // sheet names that end with # will be checked for update tags
     let reUpdateMarker = /.+ #/g
     return reUpdateMarker.test(sheetName)
 }
