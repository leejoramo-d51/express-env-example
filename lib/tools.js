...
const { log, banner } = require('./logger.js')
        ...
        if (usedColumns.includes(columnId)) {
            if ('value' in cell) {
                ...
                if ('displayValue' in cell && cell.value != cell.displayValue) {
                    row[cellName+ '__displayValue'] = cell.displayValue
                }
                if ('hyperlink' in cell && 'url' in cell.hyperlink) {
                    row[cellName+'__hyperlink'] = cell.hyperlink.url
                }
            }
            row['rowId'] = rowId
        }
...
module.exports.requestData = function (req) {
    return {
        body: req.body ?? {},
        params: req.params ?? {},
        query: req.query ?? {}
...
function smartSheetClientConfig() {
    let clientConfig = config.smartSheetAPI
    clientConfig.accessToken = decrypt(config.secret.smartSheet)
    return clientConfig
}
module.exports.smartSheetClientConfig = smartSheetClientConfig